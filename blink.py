...
data_sig,time_min,time_max,final_blinks_t,final_blinks_val,list_of_files,file_stim,file_sig,interval_corrupt,gt_blinks = ([],)*10
file_idx=fs=chan_id=flag_soft=blink_len_max=blink_len_min=delta_init=corr_threshold_1=corr_threshold_2=std_threshold_window=mode=data_len = 0
data_path=''
mode1_path = 'data/EEG-IO'
mode0_path = 'data/EEG-VV'
mode1_split = '_'
mode0_split = 'V_'

# Algorithm Parameters

def init_params(mode1, file_idx1, fs1, chan_id1, flag_soft1, blink_len_max1, blink_len_min1, delta_init1, corr_threshold_1_1, corr_threshold_2_1):
    global mode, data_path
    global file_idx,fs,chan_id,flag_soft,blink_len_max,blink_len_min,delta_init,corr_threshold_1,corr_threshold_2,std_threshold_window
    mode = mode1 # mode True means EEG-IO, otherwise v/r (EEG-VV or EEG-VR) data
    data_path = mode1_path if mode else mode0_path # or replace w/ EEG-VR
    file_idx = file_idx1

    fs = fs1
    chan_id = chan_id1

    flag_soft = flag_soft1 # if True, consider soft blinks as ground truth

    blink_len_max = blink_len_max1 # in seconds
    blink_len_min = blink_len_min1 # in seconds

    delta_init = delta_init1 # in uvolts

    corr_threshold_1 = corr_threshold_1_1
    corr_threshold_2 = corr_threshold_2_1
    #%  in seconds - for each direction
...
# Loading Data
def load_data(mode1):
    global list_of_files, file_stim, file_sig, data_sig, time_min, time_max, data_len

    list_of_files = [f for f in os.listdir(data_path) if os.path.isfile(os.path.join(data_path, f)) and '_data' in f]
    if mode1 is True:
        list_of_files_keys = [int((i.split('S')[1]).split(mode1_split)[0]) for i in list_of_files]
    else:
        list_of_files_keys = [int((i.split('S')[1]).split(mode0_split)[0]) for i in list_of_files]
    list_of_files = [x for _,x in sorted(zip(list_of_files_keys,list_of_files))]
...
def compute_final_blinks():
    global delta_init, args_chan1, running_std, min_pts, p_blinks_t, p_blinks_val, corr_matrix, pow_matrix, blink_fp_idx, t, blink_index
    global blink_template_corrmat, blink_template_powmat, blink_templates_corrWpower, final_blinks_t, final_blinks_val
    global blink_var, Z, groups, grp_1_blinks_var, grp_2_blinks_var, selected_group, template_blink_idx, delta_new
    global s_fc, sort_idx, blink_index1, blink_index2, blink_index3
    ...
    return final_blinks_t

def plot_data():
    global fig, ax1, pred, blink_start, blink_end, corrupt_start, corrupt_end, flag, ground_truth, cmat, flag_soft, ax1, total_pred ,total_gt
...
def init_data(mode1, file_idx1, fs1, chan_id1, flag_soft1, blink_len_max1, blink_len_min1, delta_init1, corr_threshold_1_1, corr_threshold_2_1):
    global interval_corrupt, gt_blinks, data_final_blink
    init_params(mode1, file_idx1, fs1, chan_id1, flag_soft1, blink_len_max1, blink_len_min1, delta_init1, corr_threshold_1_1, corr_threshold_2_1)
    load_data(mode1)
    interval_corrupt, gt_blinks = decode_stim(data_path, file_stim)
    data_final_blink = compute_final_blinks()
    return data_final_blink, data_sig, interval_corrupt

indices, datasignal, interval_corrupt=init_data(True, 0, 250, 1, True, 2.0, 0.2, 100, 0.2, 0.7)
# print(indices)
# plot_data()
#print(indices)
